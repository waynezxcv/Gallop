...
* 滚动列表的性能优化。Gallop使用异步绘制、视图层级合并、观察mainRunloop、对布局模型预先缓存等方法，能在实现复杂的图文混排界面时，仍然保持一个相当优秀的滚动性能（FPS基本保持在60）。**项目内有使用Gallop构建的微信朋友圈Demo**
* 实现图文混排界面，比如在文本中添加表情，对文字添加点击链接。
* 简便地实现对网络图片和本地图片的圆角和模糊处理，并能提供缓存，无需重复处理，优化性能。
