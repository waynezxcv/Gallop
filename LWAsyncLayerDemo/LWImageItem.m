...
- (void)setImageModel:(LWImageModel *)imageModel {
...
- (void)loadHdImageWith:(NSInteger)index animate:(BOOL)animate {
    if (self.imageModel.thumbnailImage != nil) {
        ...
        if (!self.imageModel.isDownload) {
            ...
            if (animate) {
                self.imageView.frame = self.imageModel.originFrame;
...
- (void)downloadImageWithDestinationRect:(CGRect)destinationRect {
    __weak LWImageItem* weakSelf = self;
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5f * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [manager downloadImageWithURL:[NSURL URLWithString:self.imageModel.HDURL] options:options progress:^(NSInteger receivedSize, NSInteger expectedSize) {
...
- (CGRect)calculateDestinationFrameWithSize:(CGSize)size{
    CGRect rect = CGRectMake(0.0f, (SCREENHEIGHT - size.height * SCREENWIDTH/size.width)/2, SCREENWIDTH , size.height * SCREENWIDTH/size.width);
    return rect;
}
...
- (void)handleLongpress:(UILongPressGestureRecognizer *)longpress {
    switch (longpress.state) {
        case UIGestureRecognizerStateBegan: {
            UIActionSheet* actionSheet = [[UIActionSheet alloc] initWithTitle:@"保存图片" delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:nil otherButtonTitles:@"保存到本地", nil];
